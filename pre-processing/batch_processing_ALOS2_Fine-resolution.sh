...
do
	...
	gdalwarp -overwrite -te -38.28 66.32 -38.07 66.41 temp.tif crop.tif 
	echo "python ~/zez_code/medianBlur_16.py --input crop.tif --output crop_desample.tif"
	python ~/zez_code/medianBlur_16_ALOS.py --input crop.tif --output $crop/${date}_ALOS2_helheim.tif
	echo "gdalcopyproj.py crop.tif $crop/${date}_ALOS2_helheim.tif"
	gdalcopyproj.py crop.tif $crop/${date}_ALOS2_helheim.tif
