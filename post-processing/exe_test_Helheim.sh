...
para_file=para.ini
para_py=/Users/EnzeZhang/data/Front_DL3/script/parameters.py
...
#python /home/zez/data2/pytorch_deeplab/pytorch-deeplab-xception/inference.py ${work_root} ${work_root}/para.ini ${work_root}/list/test.txt --backbone resnet --workers 1 --cuda --useBN --batchSize 1 --resume ${work_root}/resnet_linear_ALOS1_JI_Kanger_May22.tar
#python /home/zez/data2/pytorch_deeplab/pytorch-deeplab-xception/inference.py ${work_root} ${work_root}/para.ini ${work_root}/list/test.txt --backbone mobilenet --workers 1 --cuda --useBN --batchSize 1 --resume ${work_root}/mobilenet_linear_ALOS1_JI_Kanger_May21.tar
python ${work_root}/pytorch-deeplab-xception/inference.py ${work_root} ${work_root}/para.ini ${work_root}/list/test.txt --backbone drn --workers 1 --cuda --useBN --batchSize 1 --resume ${work_root}/drn_default_hist_eq_ALOS1_JI_Kanger_May8.tar
...
while (($i<$count))
    do
        ...
        python /home/zez/test_deep_learning/u_net/Unet_pytorch-master_Dec5/point_in_polygon.py --input ${work_root}/post_process_gmt/${temp2}.gmt --polygon ${work_root}/post-processing/polygon_for_truncation/in_polygon_Helheim_ENVISAT.gmt --output ${work_root}/in_polygon_gmt/${temp2}.gmt
