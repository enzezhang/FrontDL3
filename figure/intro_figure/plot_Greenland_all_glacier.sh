...
echo -41.5 66.37 Helheim |gmt pstext -J$J -R$R -O -F+f18p,0,black -K -Wwhite -Gwhite -C10%/10% -TO >>$PS
echo -58 68.6 Jakobshavn Isbr@e |gmt pstext -J$J -R$R -O -F+f18p,0,black -K -Wwhite -Gwhite -C10%/10% -TO >>$PS
echo -34 69.3 Kangerdlugssuaq |gmt pstext -J$J -R$R -O -F+f18p,0,black -K -Wwhite -Gwhite -C10%/10% -TO >>$PS

echo 69.214 -49.715 | awk '{print $2,$1}' |gmt psxy -R$R -J$J -O -K -Sa0.5 -Gblack >>$PS
echo 69.157	-49.623 | awk '{print $2,$1}' |gmt psxy -R$R -J$J -O -K -Sa0.5 -Gblack >>$PS
echo 68.574	-32.477 | awk '{print $2,$1}' |gmt psxy -R$R -J$J -O -K -Sa0.5 -Gblack >>$PS
echo 66.356	-38.188 | awk '{print $2,$1}' |gmt psxy -R$R -J$J -O -K -Sa0.5 -Gblack >>$PS
gmt psbasemap -R$R -J$J -O -P -K -Ba0 >>$PS

# plot JI using TSX images
R=-49.81/-49.5/69.11/69.26
J=M8.3
imagename=$path/20130724_JI_Landsat7-slc.grd
calving_line=$path/20130724_JI_Landsat7-slc.gmt
...
gmt psbasemap -J$J -R$R -K -V -O -P -Ba0 -Y3.88i >> $PS
